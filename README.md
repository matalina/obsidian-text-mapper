...
-   Namespaced all element IDs: HTML/SVG assumes that a DOM element `id` is unique. Prior to this change, we did not ensure unique IDs, which led to issues when there was more than one map in the same document.
    ...
    -   **Interactive canvas**: Maps are now interactive with pan and zoom support.
    -   **Pan**: Click and drag to pan around the map
    -   **Zoom**: Use the mouse wheel to zoom in/out (zoom range: 0.5x to 4.0x)
    -   **Touch support**: Pan and zoom work on touch devices
    -   **Theme support**: Three themes are available: `gnomeyland`, `apocalypse`, and `tag-and-tally`.
        -   Set a default theme in Settings → Text Mapper Settings
        -   Override per-map with `option theme <name>` (e.g., `option theme apocalypse`)
    -   **Save canvas as PNG**: Right-click on any map and select "Save canvas as PNG" to export the full map as an image file.
        -   Save location can be configured in Settings → Text Mapper Settings
        -   Default location: `_saved-maps` folder
    -   **View centering options**:
        -   `option centered-at <x> <y>` - Center the initial view on a specific hex coordinate
        -   Format: `option centered-at 0402` (4-digit) or `option centered-at 4 2` (separate numbers)
    -   `option center-content` - Center the initial view on the center of all map content (not on a specific hex)
    -   **Zoom option**: Set a default zoom level for the map with `option zoom <value>`
        -   Example: `option zoom 1.5` sets initial zoom to 150%
        -   Valid range: 0.5 to 4.0 (values outside this range will be clamped)
...
Here's an example showing some of the available options:

````
```text-mapper
option theme tag-and-tally
option zoom 1.5
option center-content
0101 plains
0202 forest
0303 mountain
0404 swamp
...
This example:
- Uses the "tag-and-tally" theme
- Sets initial zoom to 1.5x (150%)
- Centers the view on the center of all content rather than a specific hex

#### Available Options

All options are specified with the `option` keyword:

- `option horizontal` - Use pointy-top hexes instead of flat-top
- `option swap-even-odd` - Swap the even/odd positioning scheme
- `option coordinates-format {X}.{Y}` - Custom format for coordinate labels
- `option global` - Disable ID namespacing (not recommended for multiple maps)
- `option theme <name>` - Set theme: `gnomeyland`, `apocalypse`, or `tag-and-tally`
- `option zoom <value>` - Set initial zoom level (0.5 to 4.0)
- `option center-content` - Center view on content bounds center
- `option centered-at <x> <y>` - Center view on specific hex coordinate
